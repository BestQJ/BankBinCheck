...
#import <sqlite3.h>
#import "BankBinModel.h"

@interface BankBinManager ()

@property (nonatomic, assign) sqlite3 *database;

@property (nonatomic, strong) NSArray *bankNameArr;

@property (nonatomic, strong) NSArray *bankBinArr;

@end

@implementation BankBinManager

static BankBinManager *sharedInstance;
...
+ (BankBinManager *)sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        if (!sharedInstance) {
            sharedInstance = [[BankBinManager alloc] init];
            [sharedInstance saveBaseData];
        }
    });
...
- (void)saveBaseData {
    ...
    NSString *sqlPath = [docPath stringByAppendingPathComponent:@"/bankbin.sqlite"];
    NSFileManager *mannager = [NSFileManager defaultManager];
    if ([mannager fileExistsAtPath: sqlPath]) { return; }
    [self createTable];
    for (int i = 0; i < self.bankNameArr.count; i++) {
        BankBinModel *model = [[BankBinModel alloc] init];
        model.bankId = i + 10000;
        model.bankName = self.bankNameArr[i];
        model.bankBin = self.bankBinArr[i];
        [self insertData: model];
    }
}

#pragma mark - 校验银行卡合法性
...
#pragma mark - 根据银行卡号前六位（八位）获取银行名称
- (NSString *)getBankName:(NSString *)cardNo {
    if(cardNo == nil || cardNo.length < 6 || cardNo.length > 19) { return @""; }
    //6位Bin号
    NSString *cardbin_6 = [cardNo substringWithRange: NSMakeRange(0, 6)];
    NSString *selectResult = [self selectDataWithBin: cardbin_6];
    if (selectResult.length != 0) {
        return selectResult;
    }
    //8位Bin号
    NSString *cardbin_8 = [cardNo substringWithRange: NSMakeRange(0, 8)];
    selectResult = [self selectDataWithBin: cardbin_8];
    if (selectResult.length != 0) {
        return selectResult;
    }
    return @"";
...
- (BOOL)openDatabase {
    ...
    int result = sqlite3_open([sqlPath UTF8String], &_database);
    if (result != SQLITE_OK) { NSLog(@"数据库打开失败"); }
...
- (void)closeDatbase {
    if (_database != nil) {
        ...
        if (result != SQLITE_OK) { NSLog(@"数据库关闭失败"); }
        _database = nil;
    }
}

- (void)deleteTable {
    NSString *sql = @"drop table if exists bankbin;";
    if ([self openDatabase]) {
        int result = sqlite3_exec(_database, sql.UTF8String, nil, nil, nil);
        if (result == SQLITE_OK) { NSLog(@"删除数据表成功"); }
    }
}

- (void)createTable {
    NSString *sql = @"create table if not exists bankbin (ID integer primary key  autoincrement not null, bankname text not null, bankbin text)";
    if ([self openDatabase]) {
        int result = sqlite3_exec(_database, sql.UTF8String, nil, nil, nil);
        if (result != SQLITE_OK) { NSLog(@"创建表失败"); }
    }
    [self closeDatbase];
}


- (void)insertData:(BankBinModel *)model {
    if ([self selectData: model] != nil) { return; }
    NSString *sqlite = [NSString stringWithFormat:@"insert into bankbin(ID, bankname, bankbin) values ('%ld', '%@','%@')", model.bankId, model.bankName, model.bankBin];
    if ([self openDatabase]) {
        //执行sqlite语句失败的时候,会把失败的原因存储到里面
        char *error = NULL;
        int result = sqlite3_exec(_database, [sqlite UTF8String], nil, nil, &error);
        if (result != SQLITE_OK) { NSLog(@"添加数据失败"); }
    }
    [self closeDatbase];
}

- (void)deleteData:(BankBinModel *)model  {
    if (![self selectData: model]) { return; }
    NSString *sqlite = [NSString stringWithFormat:@"delete from bankbin where ID = '%ld'",model.bankId];
    if ([self openDatabase]) {
        //执行sqlite语句失败的时候,会把失败的原因存储到里面
        char *error = NULL;
        int result = sqlite3_exec(_database, [sqlite UTF8String], nil, nil, &error);
        if (result != SQLITE_OK) { NSLog(@"删除数据失败%s",error); }
    }
}

- (void)updataData:(BankBinModel *)model {
    if (![self selectData: model]) { return; }
    NSString *sqlite = [NSString stringWithFormat:@"update bankbin set bankname = '%@',bankbin = '%@' where ID = '%ld'", model.bankName, model.bankBin, model.bankId];
    if ([self openDatabase]) {
        //执行sqlite语句失败的时候,会把失败的原因存储到里面
        char *error = NULL;
        int result = sqlite3_exec(_database, [sqlite UTF8String], nil, nil, &error);
        if (result != SQLITE_OK) { NSLog(@"修改数据失败%s",error); }
    }
}
...
- (NSMutableArray *)selectAllData {
    ...
    if ([self openDatabase]) {
        NSString *sqlite = [NSString stringWithFormat:@"select * from bankbin"];
        sqlite3_stmt *stmt = NULL;
        int result = sqlite3_prepare(_database, sqlite.UTF8String, -1, &stmt, NULL);
        if (result == SQLITE_OK) {
            while (sqlite3_step(stmt) == SQLITE_ROW) {
                BankBinModel *model = [[BankBinModel alloc] init];
                model.bankId = sqlite3_column_int(stmt, 0);
                model.bankName = [NSString stringWithUTF8String:(const char *)sqlite3_column_text(stmt, 1)];
                model.bankBin = [NSString stringWithUTF8String:(const char *)sqlite3_column_text(stmt, 2)];
...
- (BankBinModel *)selectData:(BankBinModel *)model {
    NSString *sqlite = [NSString stringWithFormat:@"select * from bankbin where ID = '%ld'", model.bankId];
    BankBinModel *resultModel = nil;
    if ([self openDatabase]) {
        sqlite3_stmt *stmt = NULL;
        int result = sqlite3_prepare(_database, sqlite.UTF8String, -1, &stmt, NULL);
        if (result == SQLITE_OK) {
            while (sqlite3_step(stmt) == SQLITE_ROW) {
                resultModel = [[BankBinModel alloc] init];
                resultModel.bankId = sqlite3_column_int(stmt, 0);
                resultModel.bankName = [NSString stringWithUTF8String:(const char *)sqlite3_column_text(stmt, 1)];
                resultModel.bankBin = [NSString stringWithUTF8String:(const char *)sqlite3_column_text(stmt, 2)];
            }
        }
        sqlite3_finalize(stmt);
        [self closeDatbase];
...
- (NSString *)selectDataWithBin:(NSString *)bankbin {
    NSString *sqlite = [NSString stringWithFormat:@"select * from bankbin where bankbin = '%@'", bankbin];
    BankBinModel *resultModel = nil;
    if ([self openDatabase]) {
        sqlite3_stmt *stmt = NULL;
        int result = sqlite3_prepare(_database, sqlite.UTF8String, -1, &stmt, NULL);
